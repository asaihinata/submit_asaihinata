...
class Boxplot(twoDElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.data,label=self.label,xlabel=self.xlabel,ylabel=self.ylabel,width=self.width,whis=self.whis,fill=self.fill,showfliers=self.showfliers,notch=self.notch,orientation=self.orientation)
 def plot(self,data,label=None,xlabel=None,ylabel=None,width=0.15,whis=1.5,fill=False,legend=True,showfliers=True,notch=False,orientation='vertical'):
  self.clear()
  self.graphdata=[self.ax.boxplot(data.T,showfliers=showfliers,label=label,widths=width,whis=whis,patch_artist=fill,notch=notch,orientation=orientation)]
