...
class Bubble(twoDElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.x,self.y,self.datas,xlabel=self.xlabel,ylabel=self.ylabel,marker=self.marker,alpha=self.alpha,label=self.label)
 def plot(self,x,y,data,xlabel=None,ylabel=None,marker=None,alpha=0.5,label=None):
  self.clear()
 ...
 def update(self,x=None,y=None,data=None,**kw):
  ...
  self.plot(self.x,self.y,xlabel=self.xlabel,ylabel=self.ylabel,marker=self.marker,alpha=self.alpha,label=self.label)
  self._redraw()
