...
class Linefill(twoDElement):
 def __init__(self,master,kw):
  ...
  self.x=self._onearr(kw.get('x'),False)[0]
  ...
  self.plot(self.x,self.ymax,self.ymin,xlabel=self.xlabel,ylabel=self.ylabel,alpha=self.alpha,linewidth=self.linewidth,centerlinewidth=self.centerlinewidth)
 def plot(self,x,ymax,ymin,xlabel=None,ylabel=None,alpha=0.5,linewidth=0,centerlinewidth=2):
  self.clear()
 ...
 def update(self,x=None,ymax=None,ymin=None,**kw):
  self._updates(**kw)
  if isinstance(x,NpArraytype):self.x=self._onearr(x,False)[0]
  ...
  self.plot(self.x,self.ymax,self.ymin,xlabel=None,ylabel=None,alpha=self.alpha,linewidth=self.linewidth,centerlinewidth=self.centerlinewidth)
  self._redraw()
