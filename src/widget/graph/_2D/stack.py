...
class Stack(twoDElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.x,self.y,xlabel=self.xlabel,ylabel=self.ylabel,hatch=self.hatch,baseline=self.baseline)
 def plot(self,x,y,xlabel=None,ylabel=None,hatch=None,baseline='zero'):
  self.clear()
  self.graphdata=[self.ax.stackplot(xs,ys,color=self.colorlist[i],hatch=hatch[i],baseline=baseline)for i,(xs,ys) in enumerate(product(x,y))]
