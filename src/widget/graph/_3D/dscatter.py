...
class DScatter(threeDElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.x,self.y,self.z,xlabel=self.xlabel,ylabel=self.ylabel,zlabel=self.zlabel,marker=self.marker,alpha=self.alpha,label=self.label)
 def plot(self,x,y,z,label=None,xlabel=None,ylabel=None,zlabel=None,marker='o',alpha=1):
  self.clear()
  ...
  self._apply_labels(xlabel,ylabel,zlabel)
  self.legend()
 def update(self,x=None,y=None,z=None,**kw):
  ...
  self.label=self.labels(kw.get('label',self.label))
  self.alpha=range_num(num0s(kw.get('alpha',self.alpha),1),0,1,self.alpha)
  self.plot(self.x,self.y,self.z,xlabel=self.xlabel,ylabel=self.ylabel,zlabel=self.zlabel,marker=self.marker,alpha=self.alpha,label=self.label)
  self._redraw()
