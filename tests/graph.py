...
sgg.Hist(data=LIST(random.normal(loc=50,scale=10,lenght=1000)).flatten(),title='ヒストグラフ')
],
[
sgg.Stem(x=x1,y=y1,title='ステムグラフ'),
sgg.Boxplot(data=LIST(random.normal(loc=100,scale=10,lenght=10)).flatten(),title='箱ひげ図',whis=1.5)
],
[
sgg.Waterfall(x=x5,y=y5,width=0.5,title='ウォーターフォール',linestyles='dotted'),
...
win:WindowController=sgg.window(title='デモ',layout=layout,scroll_x=True,scroll_y=True,maxmine=True)
win.run()