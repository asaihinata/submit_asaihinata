...
[sgg.Images(path='../docs/Lenna.png')],
...
[sgg.Buttons(text='Popuptrycancel(bool型を返す)',function=lambda:print(sgg.Popuptry(message='メッセージ')))],
[sgg.LineGraph(x=x1,y=y1,title='折り線グラフ',xlabel='xlabel',ylabel='ylabel'),sgg.Pie(data=y3,title='円グラフ',label=x3)],
[sgg.BarGraph(x=x2,y=y2,title='棒グラフ(縦)',xlabel='xlabel',ylabel='ylabel',width=0.5),sgg.BarhGraph(x=x2,y=y2,title='棒グラフ(横)',xlabel='xlabel',ylabel='ylabel',height=0.5)],
[sgg.Scatter(x=x3,y=y3,title='散布図',xlabel='xlabel',ylabel='ylabel'),sgg.Scatter(x=x4,y=y4,title='散布図')],
[sgg.DScatter(x=x3D,y=y3D,z=z3D,title='3D 散布図',xlabel='x',ylabel='y',zlabel='z'),sgg.Hist(data=LIST(random.normal(loc=50,scale=10,lenght=1000)).flatten(),title='ヒストグラフ')],
[sgg.Stem(x=x1,y=y1,title='ステムグラフ'),sgg.Boxplot(data=LIST(random.normal(loc=100,scale=10,lenght=10)).flatten(),title='箱ひげ図',whis=1.5)],
[sgg.Waterfall(x=x5,y=y5,width=0.5,title='ウォーターフォール',linestyles='dotted'),sgg.Waterfallh(x=x5,y=y5,height=0.5,title='ウォーターフォール')],
[sgg.Step(data=stepdata,title='階段グラフ'),sgg.Stack(x=x6,y=y6,title='積み上げグラフ')],
